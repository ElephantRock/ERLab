...
async def test_one_succeeds_while_another_hangs():
    ...
    async def _track_and_maybe_hang(idx, proposal, ctx, provider, source_papers, source_ids, context_window, experiment_context=None, result_markers=None):
        call_log.append(idx)
        if idx == 0:
...
async def test_successful_paper_remains_available():
    ...
    async def _synthesize(idx, proposal, ctx, provider, source_papers, source_ids, context_window, experiment_context=None, result_markers=None):
        if idx == 0:
            metadata = stage._get_metadata(proposal)
...
async def test_previously_completed_work_not_repeated():
    ...
    async def _track(idx, proposal, ctx, provider, source_papers, source_ids, context_window, experiment_context=None, result_markers=None):
