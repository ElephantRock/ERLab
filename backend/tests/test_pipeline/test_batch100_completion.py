...
def test_100_02_manifest_files_exist():
    """SOUL.md and SKILL.md exist in project root (local dev only)."""
    # These are gitignored development artifacts — skip if absent
    if not (PROJECT_ROOT / "SOUL.md").exists():
        pytest.skip("SOUL.md is gitignored and not present in CI clones")
    if not (PROJECT_ROOT / "SKILL.md").exists():
        pytest.skip("SKILL.md is gitignored and not present in CI clones")
