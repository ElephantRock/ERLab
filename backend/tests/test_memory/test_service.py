...
class TestMemoryService:
    ...
    def test_recall_filters_by_type(self, memory):
        run(memory.store(MemoryEntry(
            id="", content="A research fact",
            memory_type=MemoryType.SEMANTIC, namespace="research_facts",
            truth=TruthValue.from_observation(0.8),
        )))
        run(memory.store(MemoryEntry(
            id="", content="A pipeline experience",
            memory_type=MemoryType.EPISODIC, namespace="pipeline_experience",
            truth=TruthValue.from_observation(0.8),
        )))
    ...
    def test_recall_filters_by_confidence(self, memory):
        run(memory.store(MemoryEntry(
            id="", content="Low confidence fact",
            memory_type=MemoryType.SEMANTIC, namespace="research_facts",
            truth=TruthValue(frequency=0.5, confidence=0.05),
        )))
        run(memory.store(MemoryEntry(
            id="", content="High confidence fact",
            memory_type=MemoryType.SEMANTIC, namespace="research_facts",
    ...
    def test_consolidate_merges_similar(self, memory):
        run(memory.store(MemoryEntry(
            id="", content="RAG improves retrieval accuracy by 15 percent",
            memory_type=MemoryType.SEMANTIC, namespace="research_facts",
            truth=TruthValue.from_observation(0.8),
        )))
        run(memory.store(MemoryEntry(
            id="", content="RAG improves retrieval accuracy by 20 percent",
            memory_type=MemoryType.SEMANTIC, namespace="research_facts",
            truth=TruthValue.from_observation(0.9),
        )))
    ...
    def test_persistence(self, tmp_path):
        ...
        asyncio.run(svc1.store(MemoryEntry(
            id="", content="Persistent fact",
            memory_type=MemoryType.SEMANTIC, namespace="research_facts",
            truth=TruthValue.from_observation(0.8),
        )))
