...
class RevisionDirective:
    ...
    def build_revision_prompt(self) -> str:
        ...
        # Productive-1 R3: explicit structured numeric repair targets.
        ...
        lines.append(
            "5. CONCLUSION SUPPORT: every empirical claim in the ABSTRACT or CONCLUSION "
            "using language such as 'we demonstrate', 'demonstrates that', "
            "'experimental results show/indicate', or 'results show/indicate that' "
            "MUST include a supporting [RESULT-N] marker in the same claim or sentence. "
            "Do not leave empirical conclusion claims without [RESULT-N] backing."
        )
        lines.append("6. All outcome claims must reference [RESULT-N] markers.")
        lines.append("7. Do NOT change any observed metric values, RESULT markers, or SOURCE markers.")
        lines.append(
            "8. Every [RESULT-N] marker must be placed IMMEDIATELY AFTER the exact"
        ...
        if self.method_facts:
            lines.append(
                "9. The Methodology section must state each fact below VERBATIM."
