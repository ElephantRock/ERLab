...
| `bibliography` | No `bibliography` module. **Real capability exists under a different name:** the `bibtex_exporter` *module* at `backend/pipeline/export/bibtex_exporter.py` (functions `paper_to_bibtex`, `papers_to_bibtex`, `proposal_to_bibtex` — there is no `BibTeXExporter` *class*) — capability matrix row #17 = WORKING, exposed via `/runs/:id` BibTeX button | `backend/pipeline/export/bibtex_exporter.py` (HEAD) | identical in both repos; `639e1a4` "BibTeX export for papers and proposals" | **CURRENT** (different name) | preserve current — exists as `bibtex_export` module |
