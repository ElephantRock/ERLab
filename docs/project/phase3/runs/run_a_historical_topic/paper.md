...
We define the task of Neuro-Symbolic Consistency as follows. Let $\mathcal{M}$ be a generative language model. An instance of the task consists of a triplet $(\mathcal{R}, \mathcal{S}, Q)$, where:
* $\mathcal{R} = \{r_1, r_2, \dots, r_k\}$ is a set of symbolic rules or axioms defined in First-Order Logic (FOL) or a similar formalism.
* $\mathcal{S}$ is a scenario description, often synthetic, describing the initial state or context which may be counter-factual relative to the real world (e.g., "Gravity repels objects").
* $Q$ is a query or generation prompt requiring the model to produce an output $T$.

The objective is for $\mathcal{M}$ to generate text $T$ such that $T$ is linguistically fluent and logically valid with respect to $\mathcal{R}$. Specifically, $T$ must satisfy the condition $\mathcal{R} \models T$, meaning $T$ is a logical consequence of the rules $\mathcal{R}$ given the context $\mathcal{S}$. The model must suppress its internal parametric knowledge $K_{param}$ (which encodes real-world physics/logic) in favor of the provided context $\mathcal{R}$.
...
We define a base ontology $\Omega$ representing a simplified domain (e.g., kinship, physical properties, transaction logic).  We then perturb $\Omega$ to create a counter-factual ontology $\Omega'$. This involves negating standard axioms or inverting relation functions. For example, in $\Omega$, the relation `ParentOf(A, B)` implies `AncestorOf(A, B)`. In $\Omega'$, we might define a rule where `ParentOf(A, B) \implies \neg AncestorOf(A, B)`.
...
We generate scenarios $\mathcal{S}$ that populate the ontology $\Omega'$ with specific entities. The query $Q$ asks the model to predict a future state, explain an event, or complete a narrative based on $\mathcal{S}$ and $\Omega'$.

### Evaluation Metrics

We employ two distinct classes of metrics to evaluate the performance of model $\mathcal{M}$.

**Logical Validity ($L_{score}$):**
To assess if the generated text $T$ adheres to the rules $\mathcal{R}$, we utilize a symbolic verifier. Since $T$ is natural language, we first extract logical predicates using a semantic parser (or use a separate, high-precision LLM to translate $T$ into a logical form). The extracted form $T_{logic}$ is then checked for consistency with $\mathcal{R}$ using a theorem prover or a graph traversal algorithm over the KG state.
$$ L_{score} = \frac{1}{N} \sum_{i=1}^{N} \mathbb{I}(\text{Consistent}(T_{i, logic}, \mathcal{R}_i)) $$
Where $\mathbb{I}$ is the indicator function. ## Experimental Design
